...
function DashboardAdministrador() {
  ...
  const carregarDados = useCallback(async () => {
    setCarregando(true);
    try {
      const [respostaUsuarios, respostaMaterias, respostaAtividades] =
        await Promise.all([
          apiFetch("/users"),
          apiFetch("/materias"),
          apiFetch("/atividades"),
  ...
  useEffect(() => {
    async function iniciar() {
      try {
        ...
        const role = String(autenticado?.role || "")
          .trim()
          .toUpperCase();
  ...
  async function criarUsuario(dados) {
    ...
    try {
      const rota =
        dados.role === "ALUNO" ? "/register/aluno" : "/register/professor";
      const { role: _, ...corpo } = dados;
      await apiFetch(rota, { method: "POST", body: JSON.stringify(corpo) });
      await carregarDados();
      setSucesso(
        `${dados.role === "ALUNO" ? "Aluno" : "Professor"} cadastrado com sucesso.`,
      );
      return true;
    } catch (error) {
  ...
  async function excluirUsuario(user) {
    if (user.id === usuario.id) {
      setErro("Você não pode excluir sua própria conta por este painel.");
      return;
    }
    if (
      !window.confirm(`Excluir ${user.nome}? Esta ação não pode ser desfeita.`)
    )
      return;
    iniciarOperacao();
    try {
      await apiFetch(`/users/${encodeURIComponent(user.matricula)}`, {
        method: "DELETE",
      });
      await carregarDados();
  ...
  async function criarMateria(dados) {
    iniciarOperacao();
    try {
      const { professor_id, ...materia } = dados;
      await apiFetch(`/materias/${professor_id}`, {
        method: "POST",
  ...
  async function excluirMateria(materia) {
    if (
      !window.confirm(
        `Excluir a matéria ${materia.nome}? As relações vinculadas podem ser afetadas.`,
      )
    )
      return;
    iniciarOperacao();
    try {
      ...
      await carregarDados();
      setSucesso("Matéria excluída com sucesso.");
    } catch (error) {
      setErro(error.message);
    } finally {
      finalizarOperacao();
  ...
  if (!usuario || carregando)
    return (
      <main className="admin-page">
        <p className="admin-loading">Carregando painel administrativo...</p>
      </main>
    );
